...
let get_tag t =
  ...
  | _ ->
      failwith
        "Internal error: trying to get tag for non-structure or union type"
      [@coverage off]
...
let classify_params_helper typed_asm_vals return_on_stack =
  ...
  let process_one_param (int_reg_args, dbl_reg_args, stack_args)
    ...
    | Structure _ | Union _ ->
        ...
        let classes = classify_type (get_tag tacky_t) in
...
let classify_param_types type_list return_on_stack =
  ...
  in
  let int_regs = List.take (List.length ints) int_param_passing_regs in
  let dbl_regs = List.take (List.length dbls) dbl_param_passing_regs in
  int_regs @ dbl_regs

let classify_return_helper ret_type asm_retval =
  let open Assembly in
  match ret_type with
  | Types.Structure _ | Union _ ->
      let classes = classify_type (get_tag ret_type) in
      let var_name =
...
let classify_return_type = function
  ...
  | t ->
      ...
      if return_on_stack then ([ Assembly.AX ], true)
        ...
        let int_regs = List.take (List.length ints) Assembly.[ AX; DX ] in
        let dbl_regs = List.take (List.length dbls) Assembly.[ XMM0; XMM1 ] in
